...
def initialize_datasets(args, datadir='../../data/sample_data', num_pts=None, testfile='', balance=True):
    ...
    for split in splits:
        if len(datasets[split]) > 0:
            # Decide whether to load this split into RAM based on args.ram_split.
            # If "all" is in the list, load everything; if "none" is in the list, load nothing.
            ram_flag = True  # default to loading into RAM
            if 'all' in args.ram_split:
                ram_flag = True
            elif 'none' in args.ram_split:
                ram_flag = False
            else:
                # Otherwise, load into RAM only if the current split is explicitly listed.
                ram_flag = (split in args.ram_split)
                
            logger.info(f"For split '{split}', RAMdataset set to {ram_flag} based on --ram_split={args.ram_split}")
            torch_datasets[split] = ConcatDataset([
                JetDataset(
                    filename,
                    num_pts=num_pts_per_file[split][idx],
                    shuffle=shuffle[split],
                    balance=balance,
                    RAMdataset=ram_flag
                )
                for idx, filename in enumerate(datasets[split])
            ])
